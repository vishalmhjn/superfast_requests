...
USER_AGENT = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36"}
...
async def download_all_pages(timestr, list_items):
    ...
    async with aiohttp.ClientSession(connector=connector, headers=USER_AGENT, timeout=timeout) as session: #
        tasks = []
        for item in list_items:
            search_url = "https://www.google.de/search?" + item
            task_asyn = asyncio.ensure_future(download_page(session, search_url, item, timestr))
